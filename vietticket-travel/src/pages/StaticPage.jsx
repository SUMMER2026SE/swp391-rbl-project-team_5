...
const PAGES = {
  ...
  faq: {
    ...
    sections: [
      ['Tôi nhận vé khi nào?', 'Vé QR được tạo sau khi thanh toán thành công và đơn được xác nhận. Với điểm cần duyệt thủ công, tiền được ghi nhận trước; đối tác phải phản hồi trong tối đa 24 giờ và không muộn hơn giờ bắt đầu hoạt động. Nếu từ chối hoặc quá hạn, booking bị hủy và hệ thống tạo hoàn tiền bắt buộc 100% về phương thức thanh toán gốc.'],
      ['Tôi có thể hủy hoặc hoàn vé không?', 'Điều kiện phụ thuộc vào chính sách của từng gói vé. Bản demo chỉ nhận yêu cầu hủy toàn bộ booking và toàn bộ mã QR trong booking; không hủy bớt số vé, không đổi ngày/khung giờ hoặc loại vé. Nếu chính sách trừ phí, “hoàn một phần” là một phần số tiền giao dịch sau phí, không phải hủy một phần số vé. Mức hoàn dự kiến được hiển thị trước khi gửi yêu cầu.'],
      ['Tôi muốn đổi ngày hoặc giờ tham quan thì làm thế nào?', 'Phiên bản đồ án chưa hỗ trợ sửa booking đã thanh toán. Nếu booking còn đủ điều kiện hủy, bạn gửi yêu cầu hủy toàn bộ rồi tạo booking mới theo tồn kho và giá hiện tại. VietTicket Rescue chỉ là ngoại lệ khi nhà cung cấp làm gián đoạn dịch vụ và luôn cần khách xác nhận phương án thay thế.'],
      ['Thanh toán có an toàn không?', 'VietTicket chuyển giao dịch tới cổng VNPay và không lưu số thẻ ngân hàng trên hệ thống.'],
      ['Tôi có thể mua vé người lớn và trẻ em trong cùng một đơn không?', 'Bản demo xử lý mỗi loại vé thành một đơn riêng để tồn kho, mã QR, voucher và chính sách hoàn/hủy luôn rõ ràng. Bạn hoàn tất vé người lớn trước, sau đó tạo đơn vé trẻ em; lịch trình AI sẽ xếp các dòng vé theo thứ tự để bạn không bỏ sót.'],
      ['Nếu một dòng trong lịch trình AI thanh toán thất bại thì sao?', 'Mỗi dòng vé là một booking và giao dịch độc lập, không phải một đơn hàng gộp. Các dòng đã thanh toán trước đó vẫn có hiệu lực và không tự động bị hủy hoặc hoàn khi dòng sau thất bại. Bạn có thể tiếp tục booking đang giữ, đặt lại dòng đã hủy, hoặc dừng và quản lý từng booking trong Vé của tôi.'],
  ...
  terms: {
    ...
    sections: [
      ...
      ['3. Thông tin sản phẩm và giá trong bản demo', 'Mô tả, lịch hoạt động, điều kiện độ tuổi, giá bán và khả năng cung ứng do đối tác cập nhật và được hệ thống kiểm tra trước khi mở bán. Giá cuối cùng, ưu đãi và tổng tiền được hiển thị tại bước xác nhận. Bản demo chưa có mô hình thuế suất, phí dịch vụ hoặc phát hành hóa đơn VAT, vì vậy tổng tiền không được diễn giải là đã bao gồm VAT.'],
      ['4. Giữ chỗ, lịch trình và xác nhận đơn', 'Giữ chỗ chỉ có hiệu lực trong thời hạn hiển thị. Mỗi đơn trong phiên bản trình diễn chỉ gồm một loại vé; khách cần tạo các đơn riêng nếu mua nhiều loại vé cho cùng nhóm. Lịch trình AI chỉ sắp xếp các đơn độc lập theo thứ tự, không tạo một giao dịch nguyên khối và không tự hoàn tác các booking đã thanh toán khi một dòng sau thất bại. Đơn đặt vé được xác nhận sau khi hệ thống ghi nhận thanh toán hợp lệ và kiểm tra lại tồn kho. Với sản phẩm duyệt thủ công, tiền được ghi nhận trước khi đối tác duyệt; hạn phản hồi là thời điểm sớm hơn giữa 24 giờ sau thanh toán và giờ bắt đầu hoạt động, được hiển thị trong tài khoản. Nếu đối tác từ chối hoặc quá hạn, hệ thống hủy đúng booking bị ảnh hưởng, hoàn kho và tạo yêu cầu hoàn bắt buộc 100% theo phương thức thanh toán ban đầu.'],
      ...
      ['7. Hủy, đổi lịch, không đến và hoàn tiền', 'Quyền hủy và số tiền hoàn phụ thuộc chính sách được lưu cùng gói vé, thời điểm gửi yêu cầu và trạng thái sử dụng. Trong phạm vi bản demo, yêu cầu hủy luôn áp dụng cho toàn bộ booking và toàn bộ mã QR; không hỗ trợ hủy một phần số vé, đổi ngày, đổi khung giờ hoặc đổi loại vé sau thanh toán. Nếu muốn lịch khác, khách hủy toàn bộ khi còn đủ điều kiện rồi tạo booking mới theo giá và tồn kho hiện tại. “Hoàn sau khi trừ phí” là hoàn một phần giá trị giao dịch, không phải giữ lại một phần vé. Rescue chỉ là ngoại lệ vận hành khi nhà cung cấp gây gián đoạn và cần khách xác nhận. Vé đã check-in hoặc quá hạn hủy có thể không được hoàn.'],
