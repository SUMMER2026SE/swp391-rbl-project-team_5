...
Tài liệu này dùng cho bộ dữ liệu `defense-demo-v1` trên nhánh `HAnh`. Kịch bản chính kéo dài khoảng 32–38 phút, không trình bày đăng ký/đăng nhập/xác thực email. Các cửa sổ phải được đăng nhập sẵn trước khi giảng viên vào phòng.
...
- Không phụ thuộc vào thanh toán VNPay trực tiếp trong bài chính. Dùng các booking đã chuẩn bị để trình bày đủ trạng thái; giao dịch demo không phải tiền thật.
...
`demo:smoke` chạy các thao tác customer, partner, gate staff, platform staff và admin trên API thật rồi tự reset lại dữ liệu chuẩn. Kết quả cuối phải có dòng `SMOKE TEST THÀNH CÔNG` và `Dữ liệu demo đã được phục hồi nguyên trạng`.
...
|---|---|---|---|
| A | Customer | `demo.customer@vietticket.local` | `http://localhost:5173/` |
| B | Partner | `demo.partner@vietticket.local` | `http://localhost:5173/partner/dashboard` |
| C | Gate staff | `demo.gate@vietticket.local` | `http://localhost:5173/staff/checkin` |
| D | Platform staff | `demo.support@vietticket.local` | `http://localhost:5173/staff/refunds` |
| E | Admin | `demo.admin@vietticket.local` | `http://localhost:5173/admin` |
...
| Dữ liệu | Giá trị demo |
|---|---|
| Thành phố tìm kiếm | `ho chi minh` |
| Voucher | `DEMO15` — giảm 15%, tối đa 100.000đ, đơn từ 200.000đ |
| QR check-in chính | `DEMOQR-CHECKIN-01` |
| QR dự phòng | `DEMOQR-CHECKIN-02` |
| Điểm giá thấp | Bảo tàng Thành phố Hồ Chí Minh |
| Điểm cần Partner duyệt | Du thuyền Ngắm Hoàng hôn Sài Gòn |
| Điểm trọn ngày | Khu Dự trữ Sinh quyển Cần Giờ |
...
4. Mở `Bảo tàng Thành phố Hồ Chí Minh`.
...
- Tiêu đề: `DEMO NGÀY BẢO VỆ - Cần xác nhận hiệu lực vé`
...
4. Chỉ tổng doanh thu dự kiến, dải tin cậy, biểu đồ theo ngày, số booking lịch sử và nhãn `AI demo (dữ liệu mô phỏng)`.

Lời thoại chuẩn:

> Đây là pipeline ML thật chạy Random Forest kết hợp XGBoost, target là doanh thu vé thuần theo ngày sử dụng dịch vụ. Bộ demo có 90 ngày lịch sử cho ba điểm và được gắn `demo_booking_history`, nên giao diện ghi rõ AI demo. Nhóm không dùng dữ liệu mô phỏng để tuyên bố độ chính xác thực tế.
...
2. Nhập `DEMOQR-CHECKIN-01`, bấm tra cứu.
...
5. QR `DEMOQR-CHECKIN-02` là dự phòng; không quét nếu đã đủ thời gian.
...
1. Tìm ticket `DEMO NGÀY BẢO VỆ...` của thành viên 1; nếu không có, dùng `Cần hỗ trợ nhận diện mã QR tại cổng`.
2. Gửi:
...
2. Chỉ panel forecast toàn nền tảng và nhãn nguồn dữ liệu demo.
...
1. Mở `Công ty TNHH Hành trình Xanh`, xem giấy phép demo, mã số thuế, đại diện, ngân hàng; bấm duyệt.
...
1. Tạm ẩn `Bảo tàng Thành phố Hồ Chí Minh` với:
...
1. Mở `/admin/reviews`, chọn review 2 sao có nội dung `công kích cá nhân`, ẩn với:
...
3. Mở `/admin/vouchers`, tìm `DEMO15`, chỉ điều kiện min spend, phần trăm, trần giảm và hạn dùng.
...
1. Duyệt kỳ DRAFT.
2. Chọn kỳ APPROVED, ghi nhận đã trả với mã `DEMO-BANK-TRANSFER-001`.
...
Pipeline thật dùng ensemble Random Forest + XGBoost, có lag 1/7/14 ngày, rolling mean, cuối tuần, lễ Việt Nam và split validation theo thời gian. Bộ bảo vệ dùng lịch sử mô phỏng nên bắt buộc hiển thị `AI_DEMO_ENSEMBLE` và `demo_booking_history`; nhóm không dùng metric demo để cam kết production.
...
### “Refund demo có chuyển tiền thật không?”
...
- [ ] ML `/health`: model loaded, source `demo_booking_history`.
...
- [ ] Clipboard có `DEMOQR-CHECKIN-01`, `DEMO15` và các câu lý do mẫu.
